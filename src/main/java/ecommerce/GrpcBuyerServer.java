...
public class GrpcBuyerServer extends ecommerce.BuyerGrpc.BuyerImplBase{
    ...
    public void addToShoppingCart(ecommerce.addToShoppingCartRequest request,
        ...
        DynamoDBQueryExpression<Buyer> queryUser = new DynamoDBQueryExpression<>();
        queryUser.setHashKeyValues(new Buyer(request.getBuyerId()));
        PaginatedQueryList<Buyer> listUser = db.query(Buyer.class, queryUser);
        String response ="";
        Iterator<Buyer> iterUser = listUser.iterator();
        boolean loginFlag = true;
        if(!iterUser.hasNext())
        {
            response = new StringBuilder().append("No user with given buyerId.").toString();
            loginFlag = false;
        }

        while (iterUser.hasNext()) {
            Buyer currentUser = iterUser.next();
            if(!currentUser.isLoggedin())
            {
